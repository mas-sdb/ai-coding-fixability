...
**用語に関する注記：** 本ドキュメント全体を通じて、「仕様補正（correction）ループ」 と 「不具合修正（fix）ループ」を、AI 駆動のコード改善という反復プロセスを指す用語として同義で使用しています。
...
**実装時（静的）** と **実行時** という 2 つの次元に沿って整理されています。
...
AI は外側から意味論を吸収し、中心（言語仕様）に向かって理解します。

つまり、AI は外側のデータから逆算して推測する形になります。
...
人間は中心（言語仕様）から理解を外側へ広げます。

## ## 5.3 AI と人間の弱点は完全に逆方向

> AI と人間で、このレイヤーの意味論のどこが崩れると弱いかが逆になります。
...
人間によっての「言語= 仕様 + 文法」 が、AI では「言語 = 仕様 + 文法 + ツールチェーン + 実例 + コミュニティ + 文化」 となります。

人間にとっては「周辺情報」でも、
AI にとっては 意味論の一部 になる。

つまり、AI 時代の言語とは、言語仕様だけでは定義されず、コミュニティの実践によって意味を持ちます。

AI はその全体を「言語」として扱う、エコシステムとしての言語（Language-as-Ecosystem）という言語観になります。
