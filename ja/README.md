...
このリポジトリには、相互に関連する6種類のドキュメントが含まれています：
...
#### **3. 哲学的原則 ([philosophy.md](./philosophy.md))**
- **目的：** フレームワークの設計思想と長期安定性のための原則をまとめる
- **構造：** AI 支援コーディングにおける中立性、価値観の尊重、構造的焦点などの原則
- **対象読者：** フレームワークの哲学的背景を理解したい方、設計者、研究者

#### **4. 診断ツール ([diagnostic_prompt.md](./diagnostic_prompt.md))**
...
#### **5. 診断結果サンプル ([samples/](../samples/))**
- **目的：** 診断プロンプトを特定の言語構成に適用した実例
- **構造：** 言語 + バージョン + コンテキスト → 9軸診断結果
- **対象読者：** 具体例を求めるユーザー、技術スタック評価チーム

#### **6. 思考プロセス ([model-thought-process.md](./model-thought-process.md))**
